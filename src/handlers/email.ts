...
export async function handleEmailWebhook(c: Context) {
  ...
  try {
    ...
    const fetchedEmail = await fetchEmailContent(email_id, emailLog)
    if (!fetchedEmail) {
    ...
    const emailContent = fetchedEmail.content
    ...
    if (isAutoReply(fetchedEmail.headers, from)) {
      emailLog.info({ from }, 'Auto-reply detected, skipping to prevent loop')
      return c.json({ received: true, processed: false, reason: 'auto_reply' })
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// AUTO-REPLY DETECTION (prevent infinite loops with out-of-office, etc.)
// ═══════════════════════════════════════════════════════════════════════════

function isAutoReply(headers: Record<string, string>, fromAddress: string): boolean {
  ...
  // RFC 3834: Auto-Submitted header (most reliable)
  const autoSubmitted = normalizedHeaders['auto-submitted']
  if (autoSubmitted && autoSubmitted !== 'no') {
    return true // 'auto-replied', 'auto-generated', 'auto-notified'
  }

  // Microsoft: X-Auto-Response-Suppress
  if (normalizedHeaders['x-auto-response-suppress']) {
    return true
  }

  // Various auto-reply headers
  if (normalizedHeaders['x-autoreply']) return true
  if (normalizedHeaders['x-autorespond']) return true
  if (normalizedHeaders['x-autogenerated']) return true

  // Precedence header (older convention)
  const precedence = normalizedHeaders['precedence']
  if (precedence && ['bulk', 'junk', 'auto_reply', 'list'].includes(precedence)) {
    return true
  }

  // Check from address for common auto-sender patterns
  ...
  if (autoSenderPatterns.some((pattern) => fromLower.includes(pattern))) {
    return true
  }
